...
//    Look strange? Part of the generated model is clipped and replaced by the background. The
//    reason is clear: OSG will automatically compute near and far planes of the projection matrix
//    according to the bounds of scene objects, but it can never know what you have done in
//    shaders. It calculates the bounding box of the geometry referring to the vertices stored
//    in the CPU memory, and ignores position changes in the Z direction.
//    This leads to wrong near/far values that may clip the geometry in an improper way. To solve
//    this, we would better determine the bounds of special drawables by ourselves. And that is the
//    reason why we use the setInitialBound() method here.
    geom->setInitialBound( osg::BoundingBox(-1.0f,-1.0f,-100.0f, 1.0f, 1.0f, 100.0f) );

    osg::ref_ptr<osg::Texture2D> texture = new osg::Texture2D;
    texture->setImage( osgDB::readImageFile("Images/dog_right_eye.jpg") );
