...
//VBO (Vertex Buffer Objects) is used for representing dynamic geometries. Display lists are
//unsuitable here because they don't submit vertex changes to the OpenGL pipeline, unless
//users destroy the previous display list object and create a new one. This can be done by
//calling dirtyDisplayList() method, but much less efficient than using buffer objects.
//VBO provides a fast way to communicate between user applications and the GPU while
//handing vertex attributes and indices. To notify the changes of vertex data, you may just
//call dirty() method of an array object. OSG will update it for you automatically in the
//back-end rendering.
